...
function App() {
  ...
  const handleCreateRoom = () => {
    const myPeer = new Peer(undefined, {
      host: "0.peerjs.com",
      secure: true,
      port: 443, // PeerJS server mặc định sử dụng HTTPS trên cổng 443
    });
  ...
  const addVideoStream = (video, stream, userId) => {
    ...
    gridVideo.render(
      <Avatar
        ...
        viewFullScreen={ViewFullScreen}
        hanldeShareScreen={hanldeShareScreen}
        myPeerID={myPeerID}
        socket={socket}
        userId={userId}
      />
  ...
  const hanldeShareScreen = async () => {
    const peerStream = new Peer(undefined, {
      host: "0.peerjs.com",
      secure: true,
      port: 443, // PeerJS server mặc định sử dụng HTTPS trên cổng 443
    });
